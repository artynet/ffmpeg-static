...
# clone FFmpeg trunk code
if [ ! -d $BUILD_DIR/FFmpeg-git ]
then
  git clone https://github.com/FFmpeg/FFmpeg.git FFmpeg-git
else
  cd $BUILD_DIR/FFmpeg-git
  git pull
  cd $BUILD_DIR
fi
...
if [ "$platform" = "linux" ]; then
  ...
  PKG_CONFIG_PATH="$TARGET_DIR/lib/pkgconfig:/usr/lib/pkgconfig:/usr/local/lib/pkgconfig" ./configure \
    --prefix="$TARGET_DIR" \
    --extra-cflags="-I$TARGET_DIR/include -I/usr/local/cuda/include" \
    --extra-ldflags="-L$TARGET_DIR/lib -L/usr/local/cuda/lib64" \
